...
export class Cloudipsp {
    ...
    return this.__getToken__(order)
      .then((token) => {
        return this.__checkout__(token, card, order.email)
          .then((checkout) => this.__payContinue__(checkout, token));
      });
  }
  ...
  applePayToken(token: string = req('token')): Promise<Receipt> {
    ...
    return this.__order__(token)
      .then((receipt) => {
        ...
        return this.__getPaymentConfig__(receipt.currency, 'https://apple.com/apple-pay', 'ApplePay');
      })
      .then((_config) => {
        config = _config;
  ...
  applePay(order: Order = req('order')): Promise<Receipt> {
    ...
    return this.__getPaymentConfig__(order.currency, 'https://apple.com/apple-pay', 'ApplePay')
      .then((_config) => {
        config = _config;
  ...
  googlePayToken(token: string = req('token')): Promise<Receipt> {
    ...
    return this.__order__(token)
      .then((receipt) => {
        receiptFromToken = receipt;
        return this.__getPaymentConfig__(receipt.currency, 'https://google.com/pay', 'GooglePay')
      })
      .then((_config) => {
        config = _config;
  ...
  googlePay(order: Order = req('order')): Promise<Receipt> {
    ...
    return this.__getPaymentConfig__(order.currency, 'https://google.com/pay', 'GooglePay')
      .then((_config) => {
        config = _config;
  ...
  __getPaymentConfig__(currency: string, methodId: string, methodName: string): Promise<string> {
    const request = {
    ...
    return this.__callJson__('/api/checkout/ajax/mobile_pay', request)
      .then((response) => {
        ...
        if (!data) {
          throw new Error(`${methodName} is not supported for merchant ${this.__merchantId__} and currency ${order.currency}`);
        }
    ...
    return fetch(checkout.url, {
      ...
      .then((html) => {
        return this.__cloudipspView__((cloudipspView) => {
          return cloudipspView.__confirm__(checkout.url, html, this.__baseUrl__, callbackUrl);
        });
      });
  }
